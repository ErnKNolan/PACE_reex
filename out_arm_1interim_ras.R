...
#Interim 1
#The first half of schools
in1_time1_pre <- imp %>% filter(as.numeric(as.character(schoolid))<= 56) %>%
...
in1_time1 <- left_join(in1_time1_pre,school_rands,by=c("schoolid2" = "schoolid")) %>%
  rename(LD = LD.y) %>%
...
in1_time1_split <- split(x=in1_time1,f=in1_time1$.imp)

#model the first interim
in1.time1 <- brms::brm_multiple(fu_total_pa_fin | mi() ~ LD + ses + RA_name + stype + base_total_pa_fin + trial + (1| schoolid), 
                                data = in1_time1_split,cores=4,chains=4,
...
in1.time1.samp <- brms::posterior_samples(in1.time1)[,2:3] %>%
  data.frame() %>%
...
avg_comparisons(in1.time1,variables=list(LD="pairwise"),allow_new_levels=TRUE,cross=TRUE,ndraws=10000)
#Conclusion: Pr(noninferior) = 0.99
#early stopping for efficacy
# Estimate 2.5 % 97.5 %                C: LD
#50.55  32.4   67.7 mean(H) - mean(Ctrl)
#53.75  30.1   79.4 mean(L) - mean(Ctrl)
#3.13 -18.6   27.7 mean(L) - mean(H)  

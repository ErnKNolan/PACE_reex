...
#Interim 1
#The first quarter of schools
in3_time1_pre <- imp %>% filter(as.numeric(as.character(schoolid))<= 27) %>%
...
in3_time1 <- left_join(in3_time1_pre,school_rands,by=c("schoolid2" = "schoolid")) %>%
  rename(LD = LD.y) %>%
...
in3_time1_split <- split(x=in3_time1,f=in3_time1$.imp)

#model the first interim
in3.time1 <- brms::brm_multiple(fu_total_pa_fin | mi() ~ LD + ses + RA_name + stype + base_total_pa_fin + trial + (1| schoolid), 
                                data = in3_time1_split,cores=4,chains=4,
...
in3.time1.samp <- brms::posterior_samples(in3.time1)[,2:3] %>%
  data.frame() %>%
...
avg_comparisons(in3.time1,variables=list(LD="pairwise"),allow_new_levels=TRUE,cross=TRUE,ndraws=1000)
#Estimate 2.5 % 97.5 %                C: LD
#48.1 26.67   70.3 mean(H) - mean(Ctrl)
#64.3 37.53   93.6 mean(L) - mean(Ctrl)
#15.7 -9.11   44.1 mean(L) - mean(H) 
